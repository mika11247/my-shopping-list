...
create table if not exists public.recipe_items (
  ...
  item_master_id bigint references public.item_master(id),
  user_item_master_id bigint references public.user_item_master(id),
  item_type text not null check (item_type in ('ingredient', 'seasoning')),
  amount_text text not null default '',
  sort_order integer not null default 0 check (sort_order >= 0),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint recipe_items_exactly_one_master_check check (
